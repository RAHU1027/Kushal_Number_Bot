...
async def gen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cards = [f"{bin_input[:6]}{random.randint(1000000000,9999999999)}|{random.randint(1,12):02d}|{random.randint(26,31)}|{random.randint(100,999)}" for _ in range(10)]
    
    res = (f"{get_e()} 𝗕𝗜𝗡 ⇾ <code>{bin_input[:6]}</code>\n\n<code>{chr(10).join(cards)}</code>\n\n"
